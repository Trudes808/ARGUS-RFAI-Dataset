...
src = char(metadata.audioFm.AudioSourceFile);
if ~isfile(src)   % absolute path from the generating machine; fall back to the repo-root copy
    [~, nm, ext] = fileparts(src);
    src = fullfile(fileparts(mfilename("fullpath")), "..", [nm ext]);
end
